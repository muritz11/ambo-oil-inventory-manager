...
function products_table(){
    if ( $xlsx = SimpleXLSX::parse('assets/demo/ambo_products.xlsx') ) {
...
function getUsers($curUser)
{
    ...
    while ($row = $users->fetch_assoc()) {
        ...
        $ln = $row['lastName'];
        $status = $row['status'];
        $admin = $status == 'admin' ? '' : "<a class='btn btn-info' href='users.php?adm=1&id=$id'>Make Admin</a>";
        $del = $curUser == $id ? '' : "<a href='#' class='btn btn-danger' title='delete user' id='btnDel' delID='$id' delUser='$un'><i class='fa fa-trash'></i></a>";
